...
__global__ void Update_weights(int *affected, int *parent, int *offset, int *edges, unsigned int *weight,
    ...
    if (id < N)
    {
        if(affected[id] == 0) return;
        int start = offset[id];
        int end = (id == N-1) ? E : offset[id+1];
        int minCost = INT_MAX;
        int minParent = -1;
        for (int i = start; i < end; i++)
        {
            int parentNode = edges[i];
            if (Cx[parentNode] == INT_MAX || weight[i] == INT_MAX)
                continue; 
            int cost = (Cx[parentNode] - hx[parentNode]) + weight[i] + hx[id];
            if (cost < minCost)
            {
                minCost = cost;
...
int main(int argc, char *argv[]){
    ...
    *H_a0 = 0;
    ...
    while (fscanf(fdiff,"%d",&line)!=EOF)
    {
        ...
        while (WorkList_size)
        {
            markDescendent<<<(WorkList_size + threadsCnt - 1) / threadsCnt, threadsCnt>>>(D_edges, D_offset, D_weight, D_parent,
        ...
        Update_weights<<<(N + threadsCnt - 1) / threadsCnt, threadsCnt>>>(D_affected, D_parent, r_D_offset, r_D_edges, r_D_weight,
                                                                          D_hx, flag, N, E);
        gpuErrchk(cudaPeekAtLastError());
        gpuErrchk(cudaDeviceSynchronize());
